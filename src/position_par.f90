program rpmd
  ...
  if (nonlinear .eq. 0) then
     ...
     allocate(alltcfs(nestim/2, ntime,nproc), allweights(nproc), calcscounts(nproc), ptrs(nproc))
     alltcfs(:,:,:)=0.0d0
     allweights=0.0d0
     calcscounts(:)= nestim*ntime/2
     do i=1, nproc
        ptrs(i)= (i-1)*nestim*ntime/2
     end do
     call MPI_Barrier(MPI_COMM_WORLD,ierr)
     call MPI_Gatherv(totaltcf(1:nestim/2,:), nestim*ntime/2, MPI_DOUBLE_PRECISION, alltcfs(1:nestim/2,:,:), &
          calcscounts,ptrs, MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
     call MPI_Gather(totalweight, 1, MPI_DOUBLE_PRECISION, allweights, 1, MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
  end if
  ...
  if (iproc .eq. 0) then
     ...
     if (nonlinear.eq.0) then
        ...
        do i=1, ntime
           write(100,*) dt*dble(i), (totaltcf(j,i),j=1,nestim/2)
        end do
     end if
     close(100)
  ...
  deallocate(alltcfs, allweights, calcscounts, ptrs)   
  deallocate(mass)
